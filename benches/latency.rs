...
fn bench_scheduling_latency(c: &mut Criterion) {
    ...
    for batch_size in [100, 1_000, 10_000, 100_000, 1_000_000] {
        group.throughput(Throughput::Elements(batch_size as u64));

        group.bench_function(BenchmarkId::new("batch", batch_size), |b| {
            b.iter(|| {
                // Sharded counters to reduce cache contention
                let num_shards = num_threads.max(1);
                let mut shards_lat = Vec::with_capacity(num_shards);
                let mut shards_cnt = Vec::with_capacity(num_shards);
                for _ in 0..num_shards {
                    shards_lat.push(PaddedU64::new(0));
                    shards_cnt.push(PaddedU64::new(0));
                }
                let job_latencies = Arc::new(shards_lat);
                let jobs_completed = Arc::new(shards_cnt);
                    ...
                    for _ in 0..batch_size {
                        ...
                        ctx.spawn_with_counter(
                            move |_| {
                                ...
                                // Pseudo-random modification to spread contention across shards
                                let shard_idx = (latency_ns as usize) % num_shards;

                                // Unsafe get_unchecked is safe here due to modulo, but keep it safe for bench
                                lat[shard_idx]
                                    .inner
                                    .fetch_add(latency_ns, Ordering::Relaxed);
                                comp[shard_idx].inner.fetch_add(1, Ordering::Relaxed);
                                std::hint::black_box(());
                            },
                            cnt,
                        );
                ...
                for i in 0..num_shards {
                    total += job_latencies[i].inner.load(Ordering::Relaxed);
                    count += jobs_completed[i].inner.load(Ordering::Relaxed);
                }
...
fn bench_latency_scaling(c: &mut Criterion) {
    ...
    // Fixed batch size large enough to show contention
        ...
        .filter(|&&t| t <= max_threads)
    {
        group.bench_function(BenchmarkId::new("threads", threads), |b| {
            // Re-create system for each thread count
            let system = JobSystem::new(*threads);

            // Warmup
            let warmup = system.run(|| {});
            system.wait_for_counter(&warmup);

            b.iter(|| {
                // Sharded counters to reduce cache contention
                let num_shards = max_threads.max(1);
                let mut shards_lat = Vec::with_capacity(num_shards);
                let mut shards_cnt = Vec::with_capacity(num_shards);
                for _ in 0..num_shards {
                    shards_lat.push(PaddedU64::new(0));
                ...
                // Aggregate results to force evaluation
                let mut total = 0;
                let mut count = 0;
                for i in 0..num_shards {
                    total += job_latencies[i].inner.load(Ordering::Relaxed);
