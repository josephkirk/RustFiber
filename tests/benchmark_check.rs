...
fn benchmark_heap_vs_frame() {
    ...
    println!("Heap Alloc + Global Submit ({} jobs): {:?}", count, duration_heap);
    ...
    println!("Frame Alloc + Global Submit ({} jobs): {:?}", count, duration_frame);
    
    println!("Speedup: {:.2}x", duration_heap.as_secs_f64() / duration_frame.as_secs_f64());
}
